...
export function TreeMap({ trees, onSelectCoords, className, showHeat = false }: TreeMapProps) {
  ...
  return (
    <ErrorBoundary>
      <div className={cn("relative rounded-3xl overflow-hidden border border-white/10 shadow-glow group bg-[#06130a]", className)}>
        {!isMounted && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center space-y-4 bg-[#06130a]">
            <Skeleton className="w-full h-full absolute inset-0" />
            <div className="relative z-30 text-center space-y-2">
              <p className="text-[10px] font-bold text-primary animate-pulse tracking-[0.3em]">INITIALIZING_MAP_ENGINE</p>
                      ...
                      <div className="p-3 space-y-2 min-w-[160px]">
                         ...
                         <div className="space-y-1">
                            ...
                            <div className="bg-muted px-2 py-1 rounded-lg font-mono text-[9px] text-foreground/80">
                               LAT: {(tree.lat || tree.coords?.lat || 0).toFixed(4)}<br/>
                               LNG: {(tree.lng || tree.coords?.lng || 0).toFixed(4)}
                            </div>
                         </div>
